    ...
    if (pathname === '/chat') {
      ...
      if (!session) {
        ...
        if (dailyCount.count >= 15) {
          return jsonResp({
            ...
            reply: "We're seeing a lot of messages from your connection today — please call or text us directly at (781) 929-4623, or email info@steadfastseniorservices.com.",
          }, 200, origin);
        }
      ...
      if (session.message_count >= 40) {
        return jsonResp({
          ok: true,
          sessionId,
          reply: "We've covered a lot of ground in this chat! For anything further, please call or text us at (781) 929-4623, or email info@steadfastseniorservices.com and we'll pick up right where we left off.",
        }, 200, origin);
      }

      const kbRow = await env.DB.prepare('SELECT content FROM kb_content WHERE id = 1').first();
      const kbContent = kbRow?.content ||
        'No knowledge base content is configured yet. Tell the visitor to call or text (781) 929-4623 or email info@steadfastseniorservices.com, and call capture_lead with whatever contact info they give.';
